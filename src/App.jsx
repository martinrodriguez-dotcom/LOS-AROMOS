...
import { 
  ...
  AlertTriangle, TrendingUp, TrendingDown, ClipboardList, Phone, Menu, Receipt, CheckSquare, Square
} from 'lucide-react';
...
const App = () => {
  ...
  // Lógica de Negocio y Facturación
  ...
  const billingStats = useMemo(() => {
    ...
    const totalInvoiced = mpReservations
      .filter(r => r.isInvoiced)
      .reduce((acc, r) => acc + (parseFloat(r.deposit) || 0), 0);
  ...
  const handleAddBooking = async (e) => {
    ...
    try {
      ...
      setNewBooking({ bungalowId: "1", name: '', phone: '', guests: 1, checkin: '', checkout: '', deposit: 0, isDepositPaid: false, paymentMethod: 'Efectivo', isInvoiced: false });
    ...
    <div className="flex min-h-screen bg-[#F8FAFC] text-slate-900 font-sans">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden text-slate-900">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth text-slate-900">
                    ...
                    <div className="space-y-2">
                       {dailyAgenda.checkins.map(r => (
                          <div key={r.id} className="p-3 bg-emerald-50 border border-emerald-100 rounded-xl flex justify-between items-center text-slate-900">
                             <div className="truncate"><p className="font-black text-slate-700 text-sm leading-none truncate">{r.name}</p><p className="text-[9px] font-bold text-emerald-600 mt-1 uppercase">Unidad {r.bungalowId}</p></div>
                    ...
                    <div className="space-y-2">
                       {dailyAgenda.checkouts.map(r => (
                          <div key={r.id} className="p-3 bg-red-50 border border-red-100 rounded-xl flex justify-between items-center text-slate-900">
                             <div className="truncate"><p className="font-black text-slate-700 text-sm leading-none truncate">{r.name}</p><p className="text-[9px] font-bold text-red-600 mt-1 uppercase">Unidad {r.bungalowId}</p></div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-[#009EE3] p-6 md:p-10 rounded-[2.5rem] text-white shadow-xl col-span-1 md:col-span-2 flex justify-between items-center">
                     <div>
                        ...
                        <h3 className="text-4xl md:text-6xl font-black tracking-tighter mt-1">${billingStats.totalToInvoice}</h3>
                        <p className="text-[10px] font-black uppercase mt-4 text-blue-100 opacity-80 italic">*Basado en reservas confirmadas por MercadoPago</p>
                     </div>
                  ...
                  <div className="bg-emerald-500 p-6 md:p-10 rounded-[2.5rem] text-white shadow-xl flex flex-col justify-center">
                      ...
                      <h3 className="text-3xl md:text-5xl font-black tracking-tighter mt-1">${billingStats.totalInvoiced}</h3>
                  </div>
               </div>
               ...
               <div className="space-y-4">
                  {mpReservationsByDay.map(([date, items]) => (
                    <div key={date} className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden text-slate-900">
                       <button 
                         onClick={() => setExpandedBillingDate(expandedBillingDate === date ? null : date)}
                       ...
                       >
                          <div className="flex items-center gap-4">
                             ...
                             <div className="text-left">
                                <p className="font-black text-slate-800 text-lg uppercase leading-none">{date}</p>
                                <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase">{items.length} reserva(s) • Total: ${items.reduce((acc, r) => acc + parseFloat(r.deposit), 0)}</p>
                             </div>
                          </div>
                       ...
                       {expandedBillingDate === date && (
                         <div className="p-6 pt-0 border-t border-slate-50 space-y-3 animate-in fade-in slide-in-from-top-2">
                            {items.map(r => (
                              <div key={r.id} className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${r.isInvoiced ? 'bg-emerald-50/50 border-emerald-100' : 'bg-slate-50 border-slate-100'}`}>
                                 <div className="flex items-center gap-4">
                                    <button onClick={() => toggleInvoiced(r)} className={`p-1 rounded-lg transition-colors ${r.isInvoiced ? 'text-emerald-600' : 'text-slate-300 hover:text-slate-400'}`}>
                                       {r.isInvoiced ? <CheckSquare size={24} /> : <Square size={24} />}
                                    </button>
                                    <div>
                                       <p className={`font-black text-sm ${r.isInvoiced ? 'text-emerald-800 line-through opacity-60' : 'text-slate-700'}`}>{r.name}</p>
                                       <p className="text-[10px] font-bold text-slate-400 uppercase leading-none mt-1">Unidad {r.bungalowId} • Seña: ${r.deposit}</p>
                                    </div>
                                 </div>
                                 <div className="flex gap-2">
                                    <button onClick={() => generatePDF(r)} className="p-2 bg-white text-slate-400 rounded-xl shadow-sm border border-slate-100 hover:text-slate-600"><Download size={14}/></button>
                                 </div>
                              </div>
                            ))}
                         </div>
                  ...
                  {mpReservationsByDay.length === 0 && <div className="py-24 text-center text-slate-300 font-black uppercase text-xs">No hay reservas de MercadoPago registradas</div>}
               </div>
            </div>
          )}
            ...
            <div className="max-w-5xl space-y-6 md:space-y-8 animate-in text-slate-900">
               <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-[#0F172A] p-6 md:p-10 rounded-[2.5rem] text-white shadow-2xl col-span-1 md:col-span-2 flex justify-between items-center">
                     <div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-400 mb-1 leading-none">Utilidad Neta del Mes</p><h3 className="text-4xl md:text-7xl font-black tracking-tighter text-emerald-400 mt-1">${stats.netProfit}</h3></div>
                  ...
                  <div className="bg-red-500 p-6 md:p-10 rounded-[2.5rem] text-white shadow-xl flex flex-col justify-center">
                      <p className="text-[10px] font-black uppercase tracking-widest text-red-100 opacity-80 mb-1 leading-none">Egresos Totales</p><h3 className="text-3xl md:text-5xl font-black tracking-tighter text-white mt-1">${stats.totalExpenses}</h3>
                  </div>
               </div>
          ...
          {/* ... Resto de los módulos (History, Maintenance, Modals) ... */}
          {activeTab === 'history' && (
          ...
          {activeTab === 'maintenance' && (
             <div className="max-w-4xl mx-auto bg-white p-6 md:p-8 rounded-[2rem] border border-slate-200 shadow-sm animate-in text-slate-900">
                ...
                <div className="space-y-3">
                  ...
                  {maintenance.filter(m => m.status === 'pending').length === 0 && <div className="py-16 text-center opacity-30 font-black uppercase text-xs text-slate-400 leading-none">Todo en orden por hoy</div>}
                </div>
             </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[110] flex items-center justify-center p-4 text-slate-900">
          <div className="bg-white rounded-[2rem] md:rounded-[3rem] shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col md:flex-row h-auto md:h-[65vh] animate-in zoom-in-95 duration-300">
             ...
             <div className="flex-1 p-6 md:p-8 bg-white relative overflow-y-auto text-slate-900">
                ...
                <h3 className="text-sm md:text-base font-black mb-4 uppercase border-b border-slate-100 pb-2 text-slate-800">Historial de la Unidad</h3>
                <div className="space-y-2">
                  {reservations.filter(r => r.bungalowId === selectedBungalow.id).length > 0 ? (
                    reservations.filter(r => r.bungalowId === selectedBungalow.id).sort((a,b) => new Date(b.checkin) - new Date(a.checkin)).map(r => (
                      <div key={String(r.id)} className="p-3 bg-slate-50 rounded-xl border flex flex-col gap-1 text-slate-900 shadow-sm">
                        <div className="flex justify-between items-start"><span className="font-black text-xs text-slate-700 truncate max-w-[70%] leading-none">{String(r.name)}</span><span className="text-[7px] font-black bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded uppercase leading-none">{String(r.paymentMethod || 'Efectivo')}</span></div>
            ...
            <div className="flex-1 p-6 md:p-12 bg-white relative overflow-y-auto">
              ...
              <form onSubmit={handleAddBooking} className="space-y-6">
                ...
                {newBooking.paymentMethod === 'MercadoPago' && (
                  <div className="flex items-center gap-4 p-5 bg-amber-50 border border-amber-200 rounded-[2rem] animate-pulse text-slate-900">
                    <div className="w-10 h-10 bg-amber-500 rounded-full flex items-center justify-center text-white shadow-lg shrink-0"><AlertTriangle size={20} /></div>
                    <p className="text-[10px] font-black text-amber-800 uppercase tracking-tight leading-relaxed">¡Recordatorio: Al cobrar por MercadoPago es obligatorio realizar la factura!</p>
                  </div>
                )}
      ...
      {showExpenseModal && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[120] flex items-center justify-center p-4 text-slate-900">
          <div className="bg-white rounded-[2rem] md:rounded-[3rem] shadow-2xl w-full max-w-xl p-8 text-slate-900 relative animate-in zoom-in-95 duration-300">
             ...
             <h3 className="text-xl font-black mb-8 uppercase flex items-center gap-3 text-slate-800"><TrendingDown className="text-red-500"/> Nuevo Egreso</h3>
    ...
    <div onClick={onClick} className="bg-white rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-300 group flex flex-col cursor-pointer active:scale-[0.98] text-slate-900">
      <div className="p-6 md:p-8 flex-1 flex flex-col text-slate-900">
        <div className="flex justify-between items-start mb-6 text-slate-900">
          <div className={`px-4 py-1.5 rounded-full ${config.bg} ${config.text} text-[9px] font-black uppercase flex items-center gap-2 shadow-sm`}><span className={`w-2 h-2 rounded-full ${config.dot} ${data.status === 'occupied' ? 'animate-pulse' : ''}`}></span>{config.label}</div>
        ...
        {data.status === 'occupied' && reservation ? (
          <div className="space-y-3 mt-4 animate-in text-slate-900">
            ...
            <div className="flex justify-between items-end pt-3 border-t border-slate-50 text-slate-900 mt-2"><div className="flex flex-col gap-0.5"><p className="text-[7px] font-black text-slate-400 uppercase leading-none">Salida</p><p className="text-[10px] font-black text-red-500 leading-none">{String(reservation.checkout)}</p></div><div className="text-right flex flex-col gap-0.5"><p className="text-[7px] font-black text-slate-400 uppercase leading-none">Seña</p><p className="text-xs font-black text-emerald-600 leading-none">${String(reservation.deposit)}</p></div></div>
          </div>
