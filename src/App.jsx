...
import { 
  ...
  ArrowUpCircle, ArrowDownCircle, Sparkles, Hammer
} from 'lucide-react';
...
export default function App() {
  ...
  const [newMaintenance, setNewMaintenance] = useState({
    bungalowId: "", task: "", type: "Limpieza" // "Limpieza" o "Reparación"
  });
  ...
  useEffect(() => {
    ...
    return () => { unsubB(); unsubR(); unsubM(); unsubE(); unsubI(); };
  }, [user]);
  ...
  const handleAddBooking = async (e) => {
    ...
    if (hasRepair) {
        setBlockReason("Este bungalow tiene una REPARACIÓN pendiente y no puede ser reservado hasta que se solucione.");
        return;
    }
  ...
  const handleAddMaintenance = async (e) => {
    ...
    try {
        ...
        // Cambiamos el estado visual del bungalow si es reparación
        const newStatus = newMaintenance.type === 'Reparación' ? 'cleaning' : 'cleaning'; 
        await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bungalows', newMaintenance.bungalowId.toString()), { status: 'cleaning' });
  ...
  const deleteMaintenance = async (id, bId) => {
    ...
    // Si ya no quedan mantenimientos para ese bungalow, lo ponemos libre
    const remaining = maintenance.filter(m => m.bungalowId === bId && m.id !== id);
    if (remaining.length === 0) {
        await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bungalows', bId.toString()), { status: 'free' });
    }
  };
    ...
    <div className="flex min-h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-[#0F172A] text-white z-50 transition-transform lg:relative lg:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'}`}>
        ...
        <nav className="p-4 space-y-1">
          ...
          <NavItem active={activeTab === 'history'} icon={ClipboardList} label="Buscador Huéspedes" onClick={() => setActiveTab('history')} />
        </nav>
      </aside>

      {/* Main Container */}
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="bg-white border-b px-6 py-4 flex justify-between items-center z-20 shadow-sm text-slate-900">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth text-slate-900">
          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-10 animate-in">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-slate-900">
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm text-slate-900 text-left">
                  ...
                  <div className="space-y-2 text-slate-900">
                    {dailyAgenda.checkouts.map(r => (
                      <div key={r.id} className="p-3 bg-red-50 border border-red-100 rounded-xl flex justify-between items-center shadow-sm">
                        ...
                        <button onClick={() => openMaintenanceForm({ id: r.bungalowId })} className="p-2 bg-white text-amber-600 rounded-lg shadow-sm" title="A Mantenimiento"><Wrench size={14}/></button>
                      </div>
                    ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {bungalows.map(b => (
                  <BungalowCard 
                    ...
                    onWhatsApp={sendWA} onPDF={generatePDF} onClick={() => openBungalowDetail(b)}
                  />
                ))}
              </div>
            </div>
          ...
          {activeTab === 'maintenance' && (
             <div className="max-w-5xl mx-auto space-y-6 animate-in text-slate-900">
                <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm text-slate-900">
                    <h3 className="text-xl font-black mb-8 uppercase text-slate-800 leading-none">Cabañas con Mantenimiento Pendiente</h3>
                        ...
                        {bungalows.filter(b => maintenance.some(m => m.bungalowId === b.id && m.status === 'pending')).map(b => (
                            <div key={b.id} className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className="bg-amber-100 text-amber-700 text-[10px] font-black px-2 py-1 rounded-lg uppercase tracking-tighter">Acción Pendiente</span>
                                </div>
                                <div className="space-y-2 mb-6">
                                    {maintenance.filter(m => m.bungalowId === b.id && m.status === 'pending').map(task => (
                                        <div key={task.id} className="flex items-start gap-3 p-3 bg-white rounded-xl border border-slate-100">
                                            ...
                                            <button onClick={() => deleteMaintenance(task.id, b.id)} className="p-1.5 text-slate-300 hover:text-emerald-500 transition-all"><CheckSquare size={18}/></button>
                                        </div>
                                    ))}
                                </div>
                                <button onClick={() => openMaintenanceForm(b)} className="w-full py-3 bg-[#0F172A] text-white rounded-xl text-xs font-black uppercase hover:bg-slate-800 transition-all">Agregar Tarea</button>
                            </div>
                        ))}
                        {bungalows.filter(b => maintenance.some(m => m.bungalowId === b.id && m.status === 'pending')).length === 0 && (
                            <div className="col-span-full py-20 text-center opacity-30">
                                <CheckCircle2 size={48} className="mx-auto mb-4" />
                                <p className="font-black uppercase text-sm">Todas las unidades están en perfectas condiciones</p>
                            </div>
                        )}
                    </div>
                </div>
          ...
          {/* OTRAS PESTAÑAS (FINANCE, STATS, HISTORY, BILLING) - CONSERVADAS AL 100% */}
          {activeTab === 'finance' && (
            <div className="max-w-6xl mx-auto space-y-8 animate-in text-slate-900">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-[#0F172A] p-10 rounded-[3rem] text-white shadow-2xl col-span-1 md:col-span-2 relative overflow-hidden flex flex-col justify-center">
                  <div className="z-10">
                    <p className="text-[10px] font-black uppercase tracking-widest text-emerald-400 mb-1 leading-none text-white">Utilidad Neta de Caja</p>
                  ...
                  <div className="space-y-3">
                    {manualIncomes.sort((a,b) => new Date(b.date) - new Date(a.date)).map(i => (
                      <div key={i.id} className="p-5 bg-emerald-50/40 rounded-2xl border border-emerald-100 flex justify-between items-center shadow-sm">
                        <div className="text-left"><p className="font-black text-slate-800 text-sm leading-none truncate">{i.description}</p><p className="text-[9px] font-bold text-slate-400 uppercase mt-2">{formatDateDisplay(i.date)} • {i.source}</p></div>
                        <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-3">
                    {expenses.sort((a,b) => new Date(b.date) - new Date(a.date)).map(e => (
                      <div key={e.id} className="p-5 bg-red-50/40 rounded-2xl border border-red-100 flex justify-between items-center shadow-sm">
                        <div className="text-left"><p className="font-black text-slate-800 text-sm leading-none truncate">{e.description}</p><p className="text-[9px] font-bold text-slate-400 uppercase mt-2">{formatDateDisplay(e.date)} • {e.category}</p></div>
                        <div className="flex items-center gap-4">
          ...
          {activeTab === 'stats' && (
            <div className="max-w-6xl mx-auto space-y-8 animate-in text-slate-900 pb-10">
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-slate-900">
                  <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm col-span-2 text-slate-900 text-left">
                  ...
                  <div className="bg-emerald-600 p-8 rounded-[3rem] text-white flex flex-col items-center justify-center text-center shadow-xl text-white">
                     <CalendarDays size={48} className="mb-4 opacity-50 text-white" />
                     <p className="text-[10px] font-black uppercase tracking-widest opacity-80 mb-1 leading-none text-white text-white">Mejor Época / Mayor Demanda</p>
                     <h3 className="text-5xl font-black tracking-tighter uppercase mb-4 text-white text-white leading-none">{advancedStats.bestMonth}</h3>
                  </div>
               </div>
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-slate-900">
                  <div className="bg-[#0F172A] p-8 rounded-[3rem] text-white shadow-2xl text-white text-left">
                     ...
                     <div className="space-y-4">
                        {advancedStats.topClients.map((client, idx) => (
                           <div key={idx} className="flex justify-between items-center p-4 bg-slate-800/40 rounded-2xl border border-slate-700/50 text-white">
                              <div className="flex items-center gap-4 text-white">
                                 ...
                                 <div className="text-white text-left"><p className="font-black text-sm uppercase leading-none text-white">{client.name}</p><p className="text-[10px] font-bold text-slate-400 mt-1 uppercase text-slate-400 leading-none">DNI: {client.dni || 'S/D'}</p></div>
                              </div>
                  ...
                  <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm text-slate-900 text-left">
          ...
          {activeTab === 'billing' && (
            <div className="max-w-5xl mx-auto space-y-8 animate-in text-slate-900 text-left">
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-[#009EE3] p-10 rounded-[3rem] text-white shadow-xl col-span-2 flex justify-between items-center text-white">
               ...
               <div className="space-y-4 text-slate-900">
                  {mpBilling.mpByDay.map(([date, items]) => (
                    <div key={date} className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden text-slate-900">
                       <button onClick={() => setExpandedBillingDate(expandedBillingDate === date ? null : date)} className="w-full p-6 flex justify-between items-center hover:bg-slate-50 transition-colors text-slate-900">
          ...
          {activeTab === 'history' && (
            <div className="max-w-5xl mx-auto space-y-6 animate-in text-slate-900 text-left">
               ...
               <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm text-slate-900 text-left">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-900">
                     {reservations.filter(r => r.name.toLowerCase().includes(searchTerm.toLowerCase()) || (r.dni && r.dni.includes(searchTerm))).sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)).map(r => (
                        <div key={r.id} className="p-5 bg-slate-50 rounded-2xl border border-slate-100 flex justify-between items-center group hover:bg-white hover:shadow-lg transition-all duration-300 text-slate-900 shadow-sm text-left">
                           <div className="max-w-[65%] text-slate-900 text-left text-left"><p className="font-black text-slate-800 text-sm md:text-lg leading-none truncate text-left">{String(r.name)}</p><p className="text-[9px] font-black text-slate-400 uppercase mt-2 leading-none text-left">DNI: {r.dni || 'S/D'} • In: {formatDateDisplay(r.checkin)} • Unidad {r.bungalowId}</p></div>
                           <div className="flex gap-1 text-slate-900 text-left"><button onClick={() => sendWA(r)} className="p-2 bg-emerald-50 text-emerald-600 rounded-xl leading-none leading-none"><Phone size={16}/></button><button onClick={() => generatePDF(r)} className="p-2 bg-slate-100 text-slate-600 rounded-xl leading-none leading-none"><FileText size={16}/></button></div>
                        </div>
                     ))}
                  </div>
               </div>
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[200] flex items-center justify-center p-4">
            <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-lg p-10 animate-in zoom-in-95 text-slate-900">
                <div className="flex justify-between items-center mb-8">
                    <h3 className="text-2xl font-black uppercase text-slate-800 flex items-center gap-3 leading-none"><Wrench className="text-amber-500"/> Mantenimiento / Bungalow {newMaintenance.bungalowId}</h3>
                ...
                <form onSubmit={handleAddMaintenance} className="space-y-6">
                    <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                            <button type="button" onClick={() => setNewMaintenance({...newMaintenance, type: 'Limpieza'})} className={`flex-1 py-4 rounded-2xl flex items-center justify-center gap-2 font-black text-xs uppercase border-2 transition-all ${newMaintenance.type === 'Limpieza' ? 'bg-blue-600 border-blue-600 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-400'}`}><Sparkles size={16}/> Limpieza</button>
                            <button type="button" onClick={() => setNewMaintenance({...newMaintenance, type: 'Reparación'})} className={`flex-1 py-4 rounded-2xl flex items-center justify-center gap-2 font-black text-xs uppercase border-2 transition-all ${newMaintenance.type === 'Reparación' ? 'bg-red-600 border-red-600 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-400'}`}><Hammer size={16}/> Reparar</button>
                        </div>
                    </div>
                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-slate-400 uppercase leading-none">¿Qué hay que hacer?</label>
                        <textarea required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 min-h-[120px] outline-none focus:ring-2 focus:ring-amber-500 transition-all" placeholder="Ej: Cambiar canilla baño, limpiar vidrios, reparar persiana..." value={newMaintenance.task} onChange={e => setNewMaintenance({...newMaintenance, task: e.target.value})}></textarea>
                    </div>
                    {newMaintenance.type === 'Reparación' && (
                        <div className="p-4 bg-red-50 rounded-2xl border border-red-100 flex items-center gap-3">
                            ...
                            <p className="text-[10px] font-black text-red-700 uppercase leading-tight">Nota: Las reparaciones bloquean nuevas reservas de esta unidad.</p>
                        </div>
                    )}
                    <button type="submit" className="w-full py-5 bg-amber-500 text-white rounded-[2rem] font-black text-lg shadow-xl uppercase leading-none hover:bg-amber-600 transition-all">Registrar Pendiente</button>
                </form>
            </div>
        </div>
      )}

      {/* MODAL NUEVA RESERVA (CON VALIDACIÓN DE BLOQUEO) */}
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[120] flex items-center justify-center p-0 md:p-4 text-slate-900">
          <div className="bg-white rounded-none md:rounded-[4rem] shadow-2xl w-full max-w-6xl overflow-hidden flex flex-col lg:flex-row h-full md:h-[90vh] animate-in zoom-in-95 duration-300 text-slate-900">
            <div className="w-full lg:w-5/12 bg-[#0F172A] p-6 md:p-10 text-white flex flex-col border-b lg:border-b-0 lg:border-r border-slate-800 shrink-0 text-white">
              <div className="flex justify-between items-center mb-6 text-white text-white">
                <h3 className="text-xl md:text-2xl font-black uppercase leading-none text-white text-white leading-none leading-none">Disponibilidad<br/><span className="text-emerald-500 leading-none leading-none">de Unidades</span></h3>
                <div className="flex gap-2 text-white text-white">
                  <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1))} className="p-2 bg-slate-800 rounded-xl hover:bg-emerald-600 transition-all text-white leading-none"><ChevronLeft size={18}/></button>
                  <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 1))} className="p-2 bg-slate-800 rounded-xl hover:bg-emerald-600 transition-all rotate-180 text-white leading-none"><ChevronLeft size={18}/></button>
                </div>
              </div>
              <div className="text-center mb-6 text-white text-white">
                   <p className="text-xs font-black text-emerald-400 uppercase tracking-widest leading-none leading-none leading-none leading-none leading-none">{monthNames[currentMonth.getMonth()]} {currentMonth.getFullYear()}</p>
              </div>
              <div className="grid grid-cols-7 mb-4 text-center opacity-40 uppercase text-white text-white leading-none leading-none">
                {['D','L','M','M','J','V','S'].map(d => <div key={d} className="text-[8px] md:text-[10px] font-black leading-none leading-none leading-none">{d}</div>)}
              </div>
              <div className="grid grid-cols-7 gap-1.5 md:gap-3 text-white text-white">
                {(() => {
                  ...
                  for (let d = 1; d <= days; d++) {
                    ...
                    cells.push(<div key={d} className={`aspect-square flex items-center justify-center rounded-lg md:rounded-2xl text-[10px] md:text-xs font-black transition-all ${occupied ? 'bg-red-500/30 text-red-300 border border-red-500/20 line-through' : 'bg-slate-800/60 text-slate-400'} leading-none leading-none`}>{d}</div>);
                  }
                  return cells;
                })()}
              </div>
            ...
            <div className="flex-1 p-6 md:p-12 bg-white relative overflow-y-auto scroll-smooth text-slate-900 text-slate-900">
              <button onClick={() => setShowAddModal(false)} className="absolute top-4 right-4 p-3 bg-slate-50 rounded-full hover:bg-slate-200 z-10 text-slate-400 leading-none leading-none leading-none leading-none leading-none leading-none"><X size={18}/></button>
              <h3 className="text-2xl md:text-4xl font-black mb-8 uppercase text-slate-800 leading-none leading-none leading-none text-slate-800 text-slate-800">Nueva Reserva</h3>
              
              {blockReason && (
                <div className="mb-8 p-6 bg-red-50 border border-red-200 rounded-[2rem] flex items-start gap-4 animate-bounce">
                    <AlertCircle className="text-red-600 shrink-0" size={24} />
                    <p className="text-sm font-black text-red-700 uppercase leading-relaxed text-left">{blockReason}</p>
                </div>
              )}

              <form onSubmit={handleAddBooking} className="space-y-6 text-slate-900 text-slate-900">
                <div className="space-y-3 text-slate-900 text-slate-900"><label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 leading-none leading-none leading-none leading-none leading-none leading-none">Unidad</label>
                  <div className="grid grid-cols-6 gap-2 text-slate-900 text-slate-900 text-slate-900">
                    {bungalows.map(b => (
                      <button type="button" key={b.id} onClick={() => { setNewBooking({...newBooking, bungalowId: b.id}); setBlockReason(null); }} className={`h-10 md:h-12 rounded-xl text-[10px] font-black border-2 transition-all ${newBooking.bungalowId === b.id ? 'bg-[#0F172A] border-[#0F172A] text-white shadow-xl scale-105' : 'bg-slate-50 border-slate-50 text-slate-400'} leading-none leading-none leading-none leading-none leading-none`}>{String(b.id)}</button>
                    ))}
                  </div>
                </div>
                {/* Campos del Formulario de Reserva */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-900 text-slate-900">
                  <div className="space-y-1 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase leading-none ml-1 mb-1 leading-none">Huésped</label><input type="text" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-bold text-slate-800 leading-none leading-none" value={newBooking.name} onChange={(e) => setNewBooking({...newBooking, name: e.target.value})} /></div>
                  <div className="space-y-1 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase leading-none ml-1 mb-1 leading-none">DNI / Pasaporte</label><input type="text" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-bold text-slate-800 leading-none leading-none" value={newBooking.dni} onChange={(e) => setNewBooking({...newBooking, dni: e.target.value})} /></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-900 text-slate-900">
                  <div className="space-y-1 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase leading-none ml-1 mb-1 leading-none">WhatsApp</label><input type="tel" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-bold text-slate-800 leading-none leading-none" value={newBooking.phone} onChange={(e) => setNewBooking({...newBooking, phone: e.target.value})} /></div>
                  <div className="space-y-1 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase leading-none ml-1 mb-1 leading-none text-slate-900 font-black">Monto Total Estadía ($)</label><input type="number" required className="w-full p-4 bg-emerald-50 border border-emerald-100 rounded-2xl outline-none font-black text-emerald-800 leading-none leading-none" value={newBooking.totalAmount} onChange={(e) => setNewBooking({...newBooking, totalAmount: e.target.value})} /></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-900 text-slate-900">
                  <div className="space-y-1 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase font-bold leading-none">Check-In</label><input type="date" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 leading-none leading-none text-slate-900" value={newBooking.checkin} onChange={(e) => setNewBooking({...newBooking, checkin: e.target.value})} /></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 md:p-10 rounded-[2.5rem] border border-slate-100 text-slate-900 text-slate-900">
                    ...
                    {newBooking.isDepositPaid && (
                      <div className="bg-emerald-50 p-6 rounded-[2rem] border border-emerald-100 flex flex-col justify-center animate-in fade-in slide-in-from-right-4 text-slate-900 text-slate-900">
                          ...
                          <input type="number" required className="bg-transparent border-none outline-none font-black text-emerald-700 text-4xl md:text-5xl w-full leading-none leading-none" placeholder="0" value={newBooking.deposit} onChange={(e) => setNewBooking({...newBooking, deposit: e.target.value})} />
                      </div>
                    )}
                </div>
                <button type="submit" disabled={isProcessing} className="w-full py-6 md:py-8 bg-emerald-600 text-white rounded-[2rem] md:rounded-[3rem] font-black text-lg md:text-2xl shadow-2xl active:scale-95 uppercase leading-none leading-none">Confirmar Reserva</button>
              </form>
            </div>
          </div>
        </div>
      ...
      {showIncomeModal && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[150] flex items-center justify-center p-4 text-slate-900">
           <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-lg p-10 animate-in zoom-in-95 text-slate-900">
              <div className="flex justify-between items-center mb-8">
                 <h3 className="text-2xl font-black uppercase text-slate-800 flex items-center gap-3 leading-none text-emerald-600 leading-none"><ArrowUpCircle className="text-emerald-500 leading-none"/> Nuevo Ingreso</h3>
      ...
      {showExpenseModal && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[150] flex items-center justify-center p-4 text-slate-900">
          <div className="bg-white rounded-[2rem] md:rounded-[3rem] shadow-2xl w-full max-w-xl p-8 md:p-10 text-slate-900 relative animate-in zoom-in-95 duration-300 text-slate-900">
             <button onClick={() => setShowExpenseModal(false)} className="absolute top-6 right-6 p-3 bg-slate-50 rounded-full hover:bg-slate-200 transition-all text-slate-400 leading-none leading-none"><X size={18}/></button>
             <h3 className="text-xl font-black mb-8 uppercase flex items-center gap-3 text-red-500 leading-none leading-none leading-none"><TrendingDown className="text-red-500 leading-none leading-none"/> Nuevo Egreso</h3>
             <form onSubmit={handleAddExpense} className="space-y-4 text-slate-900">
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase leading-none leading-none">Descripción</label><input type="text" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 leading-none leading-none" placeholder="Ej: Pago de Luz" value={newExpense.description} onChange={(e) => setNewExpense({...newExpense, description: e.target.value})}/></div>
                <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase leading-none leading-none">Monto ($)</label><input type="number" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 leading-none leading-none" value={newExpense.amount} onChange={(e) => setNewExpense({...newExpense, amount: e.target.value})}/></div>
                   <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase leading-none leading-none">Fecha</label><input type="date" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 leading-none leading-none" value={newExpense.date} onChange={(e) => setNewExpense({...newExpense, date: e.target.value})}/></div>
                </div>
                <div className="space-y-1 text-slate-900">
                   <label className="text-[10px] font-black text-slate-400 uppercase leading-none leading-none">Categoría</label>
                   <select className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 appearance-none leading-none leading-none" value={newExpense.category} onChange={(e) => setNewExpense({...newExpense, category: e.target.value})}>
                      <option>Servicios</option><option>Mantenimiento</option><option>Limpieza</option><option>Sueldos</option><option>Otros</option>
                   </select>
                </div>
                <button type="submit" disabled={isProcessing} className="w-full py-5 bg-red-500 text-white rounded-[2rem] font-black text-lg shadow-xl uppercase leading-none leading-none">Guardar Egreso</button>
             </form>
          </div>
        </div>
      )}
      ...
      {showEditModal && resToEdit && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[150] flex items-center justify-center p-4 text-slate-900 text-slate-900 text-slate-900">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-2xl p-8 md:p-12 relative animate-in zoom-in-95 duration-300 overflow-y-auto max-h-[90vh] text-slate-900 text-slate-900 text-slate-900">
             ...
             <h3 className="text-2xl font-black mb-10 tracking-tighter uppercase flex items-center gap-3 text-slate-800 font-black border-b pb-4 leading-none leading-none leading-none text-slate-800 leading-none leading-none"><Pencil className="text-emerald-500 leading-none leading-none"/> Editar Reserva</h3>
             <form onSubmit={handleEditBooking} className="space-y-6 text-slate-900 text-slate-900 text-slate-900">
                <div className="space-y-1 text-slate-900 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase leading-none leading-none ml-1 leading-none">Huésped</label><input type="text" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 leading-none leading-none" value={resToEdit.name} onChange={(e) => setResToEdit({...resToEdit, name: e.target.value})} /></div>
                <div className="grid grid-cols-2 gap-4 text-slate-800 text-slate-800 text-slate-800">
                   <div className="space-y-1 text-slate-900 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase leading-none leading-none ml-1 leading-none">DNI</label><input type="text" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 leading-none leading-none" value={resToEdit.dni} onChange={(e) => setResToEdit({...resToEdit, dni: e.target.value})} /></div>
                   <div className="space-y-1 text-slate-900 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase leading-none leading-none ml-1 leading-none">Total Estadía ($)</label><input type="number" required className="w-full p-4 bg-emerald-50 border border-emerald-100 rounded-2xl font-black text-emerald-800 leading-none leading-none" value={resToEdit.totalAmount} onChange={(e) => setResToEdit({...resToEdit, totalAmount: e.target.value})} /></div>
                </div>
                <div className="grid grid-cols-2 gap-4 text-slate-900 text-slate-900 text-slate-900">
                   <div className="space-y-1 text-slate-900 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase leading-none leading-none ml-1 font-bold text-slate-900 leading-none leading-none">Check-In</label><input type="date" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 leading-none leading-none text-slate-900 leading-none" value={resToEdit.checkin} onChange={(e) => setResToEdit({...resToEdit, checkin: e.target.value})} /></div>
                   <div className="space-y-1 text-slate-900 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase leading-none leading-none ml-1 font-bold text-slate-900 leading-none leading-none">Check-Out</label><input type="date" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 leading-none leading-none text-slate-900 leading-none" value={resToEdit.checkout} onChange={(e) => setResToEdit({...resToEdit, checkout: e.target.value})} /></div>
                </div>
                <div className="space-y-1 text-slate-900 text-slate-900 text-slate-900"><label className="text-[9px] font-black text-slate-400 uppercase leading-none leading-none ml-1 leading-none text-slate-900 leading-none leading-none">Monto Seña entregado ($)</label><input type="number" required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 leading-none leading-none" value={resToEdit.deposit} onChange={(e) => setResToEdit({...resToEdit, deposit: e.target.value})} /></div>
                <button type="submit" disabled={isProcessing} className="w-full py-5 bg-[#0F172A] text-white rounded-[2rem] font-black text-lg shadow-xl uppercase mt-4 leading-none leading-none leading-none leading-none">Guardar Cambios</button>
             </form>
          </div>
        </div>
      )}
      ...
      {showDeleteReasonModal && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[150] flex items-center justify-center p-4 text-slate-900 text-slate-900 text-slate-900">
           <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-md p-10 text-center animate-in zoom-in-95 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
              ...
              <h3 className="text-xl font-black mb-2 uppercase text-slate-800 leading-none leading-none text-slate-800">Eliminar Reserva</h3>
              <p className="text-slate-500 text-sm mb-8 leading-relaxed leading-none leading-none text-slate-500">Indique el motivo antes de borrar a <span className="font-bold text-slate-700 leading-none leading-none leading-none">{resToDelete?.name}</span>.</p>
              <div className="space-y-3">
                 <button onClick={() => confirmDeleteReservation("Cancelación")} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black hover:bg-red-600 transition-all uppercase text-xs leading-none leading-none leading-none text-white">Cancelación Definitiva</button>
                 <button onClick={() => confirmDeleteReservation("Cambio de fechas")} className="w-full py-4 bg-white border-2 border-slate-200 text-slate-600 rounded-2xl font-black hover:border-emerald-500 hover:text-emerald-600 transition-all uppercase text-xs leading-none leading-none leading-none text-slate-600">Cambio de Fechas</button>
                 <button onClick={() => setShowDeleteReasonModal(false)} className="w-full py-2 text-slate-400 font-bold hover:text-slate-600 transition-all text-xs leading-none leading-none text-slate-400 leading-none">Cancelar</button>
              </div>
           </div>
        </div>
      )}
...
function NavItem({ icon: Icon, label, active, onClick, badge }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center justify-between px-6 py-4 rounded-xl transition-all ${active ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white'}`}>
      <div className="flex items-center gap-4 text-left leading-none"><Icon size={18} /><span className="font-bold text-sm tracking-tight leading-none leading-none">{label}</span></div>
      {badge > 0 && <span className="bg-amber-500 text-[#0F172A] text-[9px] font-black px-2 py-0.5 rounded-md leading-none leading-none">{badge}</span>}
    </button>
  );
}
...
function StatCard({ label, value, color, bg, icon: Icon, onClick }) {
  return (
    <div onClick={onClick} className={`p-4 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-4 ${bg} cursor-pointer active:scale-95 transition-all leading-none`}>
      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${color} bg-white shadow-sm shrink-0 leading-none leading-none`}><Icon size={24} /></div>
      <div className="truncate text-slate-900 leading-none"><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none truncate leading-none mb-1 text-slate-400">{label}</p><p className={`text-2xl font-black ${color} tracking-tighter truncate leading-none leading-none`}>{value}</p></div>
    </div>
  );
}
...
function BungalowCard({ data, reservation, onStatusChange, onWhatsApp, onPDF, onClick }) {
  ...
  return (
    <div onClick={onClick} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-300 group flex flex-col cursor-pointer active:scale-[0.98] text-slate-900 leading-none">
      <div className="p-8 flex-1 flex flex-col leading-none">
        <div className="flex justify-between items-start mb-6 leading-none">
          <div className={`px-4 py-1.5 rounded-full ${config.bg} ${config.text} text-[9px] font-black uppercase flex items-center gap-2 shadow-sm leading-none`}><span className={`w-2 h-2 rounded-full ${config.dot} ${data.status === 'occupied' ? 'animate-pulse' : ''} leading-none`}></span>{config.label}</div>
          <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-2 leading-none"><button onClick={(e) => { e.stopPropagation(); onStatusChange(); }} className="p-2 bg-white hover:bg-amber-50 text-amber-600 rounded-xl shadow-sm border border-slate-50 leading-none leading-none leading-none"><Wrench size={14}/></button></div>
        </div>
        <div className="mb-4 leading-none">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none mb-1 text-slate-400 leading-none">BUNGALOW</p>
        ...
        {data.status === 'occupied' && reservation ? (
          <div className="mt-4 space-y-3 animate-in leading-none text-slate-900 leading-none">
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-50 text-slate-900 leading-none leading-none"><p className="text-[8px] font-black text-slate-400 uppercase mb-1 leading-none leading-none text-slate-400 leading-none">Huésped</p><p className="text-sm font-black text-slate-700 truncate leading-none leading-none">{String(reservation.name)}</p></div>
            <div className="grid grid-cols-2 gap-2 leading-none leading-none"><button onClick={(e) => { e.stopPropagation(); onWhatsApp(reservation); }} className="flex items-center justify-center gap-1 py-2 bg-emerald-600 text-white rounded-xl text-[9px] font-black leading-none leading-none text-white leading-none"><Phone size={10} /> WA</button><button onClick={(e) => { e.stopPropagation(); onPDF(reservation); }} className="flex items-center justify-center gap-1 py-2 bg-slate-100 text-slate-600 rounded-xl text-[9px] font-black leading-none leading-none text-slate-600 leading-none"><Download size={10} /> PDF</button></div>
            <div className="flex justify-between border-t border-slate-50 pt-3 leading-none leading-none"><div className="flex flex-col leading-none text-slate-900 leading-none leading-none"><p className="text-[7px] font-black text-slate-400 uppercase leading-none leading-none text-slate-400 leading-none leading-none">Salida</p><p className="text-[10px] font-black text-red-500 leading-none leading-none leading-none leading-none">{formatDateDisplay(reservation.checkout)}</p></div><div className="text-right leading-none text-slate-900 leading-none leading-none"><p className="text-[7px] font-black text-slate-400 uppercase leading-none leading-none text-slate-400 leading-none leading-none">Seña</p><p className="text-xs font-black text-emerald-600 leading-none leading-none leading-none leading-none">${String(reservation.deposit)}</p></div></div>
          </div>
        ) : (<div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-50 rounded-2xl mt-4 bg-slate-50/50 py-10 opacity-30 text-slate-400 leading-none text-slate-400 leading-none"><Package size={24}/><p className="text-[8px] font-black uppercase mt-2 leading-none leading-none text-slate-400 leading-none">Disponible</p></div>)}
      </div>
    </div>
  );
}
