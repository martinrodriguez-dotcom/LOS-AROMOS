...
// Saneamiento de appId para evitar errores de segmentos pares
...
const App = () => {
  ...
  const generatePDF = async (res) => {
    ...
    // 1. Marca de Agua (Fondo en 45 grados)
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC] text-slate-900 font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-[#0F172A] text-white z-50 transform transition-transform duration-300 lg:relative lg:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-8 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-lg font-black tracking-tighter leading-none">LOS AROMOS</h1><p className="text-[10px] text-emerald-400 font-bold uppercase mt-1">Administración</p></div>
          </div>
        ...
        <nav className="flex-1 p-4 mt-4 space-y-1">
          <NavItem active={activeTab === 'dashboard'} icon={LayoutDashboard} label="Panel de Control General" onClick={() => { setActiveTab('dashboard'); setIsMobileMenuOpen(false); }} />
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth">
            ...
            <div className="max-w-5xl space-y-6 md:space-y-8 animate-in text-slate-900">
               <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-red-500 p-6 md:p-10 rounded-[2.5rem] text-white shadow-xl flex flex-col justify-center">
                      <p className="text-[10px] font-black uppercase tracking-widest text-red-100 opacity-80 mb-1">Gastos</p><h3 className="text-3xl md:text-5xl font-black tracking-tighter">${stats.totalExpenses}</h3>
                  </div>
               </div>
               <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                     ...
                     {expenses.length === 0 && <p className="col-span-full text-center text-slate-300 py-10 text-xs font-black uppercase">Sin egresos registrados</p>}
                  </div>
               </div>
            </div>
          )}
          ...
          {activeTab === 'history' && (
            <div className="max-w-5xl space-y-6 animate-in text-slate-900">
               <div className="bg-white p-4 md:p-6 rounded-[1.5rem] border border-slate-200 shadow-sm flex items-center gap-3 md:gap-4">
                  <Search className="text-slate-400" size={20}/><input type="text" placeholder="Buscar por nombre de huésped..." className="flex-1 bg-transparent border-none outline-none font-bold text-sm md:text-lg text-slate-800" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
               </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                     {reservations.filter(r => r.name.toLowerCase().includes(searchTerm.toLowerCase())).sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)).map(r => (
                        <div key={r.id} className="p-5 bg-slate-50 rounded-2xl border flex justify-between items-center group hover:bg-white hover:shadow-lg transition-all duration-300">
                           <div className="max-w-[65%]"><p className="font-black text-slate-800 text-sm md:text-lg leading-none truncate">{String(r.name)}</p><p className="text-[9px] font-black text-slate-400 uppercase mt-2">{r.checkin} • Unidad {r.bungalowId}</p></div>
          ...
          {activeTab === 'maintenance' && (
             <div className="max-w-4xl mx-auto bg-white p-6 md:p-8 rounded-[2rem] border border-slate-200 shadow-sm animate-in text-slate-900">
                <h3 className="text-sm md:text-xl font-black mb-6 uppercase text-slate-800">Pendientes de Servicio</h3>
                <div className="space-y-3">
                  {maintenance.filter(m => m.status === 'pending').map(m => (
                    <div key={m.id} className="p-4 bg-slate-50 rounded-xl border flex items-center justify-between">
                  ...
                  {maintenance.filter(m => m.status === 'pending').length === 0 && <div className="py-16 text-center opacity-30 font-black uppercase text-xs text-slate-400">Sin tareas pendientes</div>}
                </div>
             </div>
          )}
        </div>
      ...
      {/* MODAL DETALLE BUNGALOW (REDUCIDO Y LEGIBLE) */}
      {showDetailModal && selectedBungalow && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[110] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] md:rounded-[3rem] shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col md:flex-row h-auto md:h-[60vh] animate-in zoom-in-95 duration-300">
             <div className="w-full md:w-5/12 bg-[#0F172A] p-6 text-white flex flex-col border-b md:border-b-0 md:border-r border-slate-800 shrink-0">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-base md:text-xl font-black tracking-tight uppercase leading-none truncate pr-2">{String(selectedBungalow.name)}</h3>
                ...
                <div className="space-y-2">
                  {reservations.filter(r => r.bungalowId === selectedBungalow.id).length > 0 ? (
                    reservations.filter(r => r.bungalowId === selectedBungalow.id).sort((a,b) => new Date(b.checkin) - new Date(a.checkin)).map(r => (
                      <div key={String(r.id)} className="p-3 bg-slate-50 rounded-xl border flex flex-col gap-1">
            ...
            <div className="flex-1 p-6 md:p-12 bg-white relative overflow-y-auto text-slate-900 scroll-smooth">
              ...
              <form onSubmit={handleAddBooking} className="space-y-6 md:space-y-8">
                ...
                {newBooking.paymentMethod === 'MercadoPago' && (
                  <div className="flex items-center gap-4 p-5 bg-amber-50 border border-amber-200 rounded-[2rem] animate-pulse">
                    ...
                    <p className="text-[10px] font-black text-amber-800 uppercase tracking-tight leading-relaxed">¡Recordatorio Administrativo: No olvides realizar la factura!</p>
                  </div>
                )}
...
const BungalowCard = ({ data, reservation, onStatusChange, onWhatsApp, onPDF, onClick }) => {
  ...
  return (
    <div onClick={onClick} className="bg-white rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-300 group flex flex-col cursor-pointer active:scale-[0.98]">
      <div className="p-6 md:p-8 flex-1 flex flex-col text-slate-900">
        ...
        <h3 className="text-xl md:text-2xl font-black text-slate-800 tracking-tighter mb-2 uppercase leading-none truncate">{String(data.name || data.id)}</h3>
        {data.status === 'occupied' && reservation ? (
          <div className="space-y-3 mt-4 animate-in">
            ...
            <div className="grid grid-cols-2 gap-2"><button onClick={(e) => { e.stopPropagation(); onWhatsApp(reservation); }} className="flex items-center justify-center gap-1.5 py-2.5 bg-emerald-600 text-white rounded-2xl text-[8px] font-black uppercase shadow-lg shadow-emerald-500/20 active:scale-95 transition-all"><Phone size={10} /> WhatsApp</button><button onClick={(e) => { e.stopPropagation(); onPDF(reservation); }} className="flex items-center justify-center gap-1.5 py-2.5 bg-slate-100 text-slate-600 rounded-2xl text-[8px] font-black uppercase active:scale-95 shadow-sm transition-all"><Download size={10} /> PDF</button></div>
            <div className="flex justify-between items-end pt-3 border-t border-slate-50 text-slate-900 mt-2"><div className="flex flex-col gap-0.5"><p className="text-[7px] font-black text-slate-400 uppercase leading-none">Salida</p><p className="text-[10px] font-black text-red-500">{String(reservation.checkout)}</p></div><div className="text-right flex flex-col gap-0.5"><p className="text-[7px] font-black text-slate-400 uppercase leading-none">Seña</p><p className="text-xs font-black text-emerald-600">${String(reservation.deposit)}</p></div></div>
          </div>
        ) : (<div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-100 rounded-2xl mt-4 bg-slate-50/50 text-slate-200 py-6"><Package size={24} className="opacity-50" /><p className="text-[8px] font-black uppercase mt-2 tracking-widest opacity-50">Disponible</p></div>)}
      </div>
    </div>
  );
};
